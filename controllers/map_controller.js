...
module.exports.chart = async (req, res) => {
    let cropData = await uttarkhand.find({ District_Name: req.body.city, Crop_Year: 2014 }).sort({ Production: -1 }).exec();
    ...
    let year13Data = await uttarkhand.find({ District_Name: req.body.city, Crop_Year: 2013 }).sort({ Production: -1 }).exec();
